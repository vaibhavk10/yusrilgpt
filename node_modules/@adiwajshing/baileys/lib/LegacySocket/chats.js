...
const makeChatsSocket = (config) => {
    ...
    const executeChatModification = (node) => {
        ...
        switch (updateType) {
            ...
            case 'pin':
                ev.emit('chats.update', [{
                    id: jid,
                    pin: attributes.pin ? +attributes.pin : null
                }]);
                break;
            case 'star':
            case 'unstar':
                const starred = updateType === 'star';
                const updates = node.content.map(({
                    attrs
                }) => ({
                    key: {
                        remoteJid: jid,
            ...
            default:
                logger.warn({
                    node
                }, 'received unrecognized chat update');
                break;
        }
    };
    ...
    const chatRead = async (fromMessage, count) => {
        ...
        if (config.emitOwnEvents) {
            ev.emit('chats.update', [{
                id: fromMessage.remoteJid,
                unreadCount: count < 0 ? -1 : 0
            }]);
        }
    };
    ...
    socketEvents.on('CB:response,type:chat', async ({
        content: data
    }) => {
    ...
    socketEvents.on('CB:response,type:contacts', async ({
        content: data
    }) => {
        if (Array.isArray(data)) {
    ...
    // read updates
    socketEvents.on('CB:action,,read', async ({
        content
    }) => {
    ...
    socketEvents.on('CB:action,,chat', ({
        content
    }) => {
        if (Array.isArray(content)) {
            const [node] = content;
    ...
    return {
        ...
        chatModify: async (modification, jid, chatInfo, timestampNow) => {
            ...
            if ('archive' in modification) {
                ...
                if (modification.clear !== 'all') {
                    data = modification.clear.messages.map(({
                        id,
                        fromMe
                    }) => ({
                        tag: 'item',
                ...
                data = modification.star.messages.map(({
                    id,
                    fromMe
                }) => ({
                    tag: 'item',
         ...
         * */
        presenceSubscribe: async (jid) => (sendNode({
            json: ['action', 'presence', 'subscribe', jid]
        })),
        ...
        updateBusinessProfile: async (profile) => {
            ...
            const json = ['action', 'editBusinessProfile', {
                ...profile,
                v: 2
            }];
            await query({
        ...
        async updateProfilePicture(jid, imgBuffer) {
            ...
            const {
                img
            } = await (0, Utils_1.generateProfilePicture)(imgBuffer);
            ...
            const {
                eurl
            } = await this.setQuery([query], [Types_1.WAMetric.picture, 136], tag);
            if (config.emitOwnEvents) {
