...
const makeSocket = ({
    ...
    const sendNode = async ({
        ...
        if (binaryTag) {
            if (Array.isArray(json)) {
                throw new boom_1.Boom('Expected BinaryNode with binary code', {
                    statusCode: 400
                });
            }
            if (!authInfo) {
                throw new boom_1.Boom('No encryption/mac keys to encrypt node with', {
                    statusCode: 400
                });
            }
        ...
        const listener = ([, connected]) => {
            if (connected) {
                timeout = setTimeout(() => {
                    ...
                    cancel(new boom_1.Boom('Not expecting a response', {
                        statusCode: 422
                    }));
                }, expectResponseTimeout);
            ...
            promise: (async () => {
                ...
                try {
                    const result = await (0, Utils_1.promiseTimeout)(timeoutMs, (resolve, reject) => {
                        onRecv = resolve;
                        onErr = err => {
                            reject(err || new boom_1.Boom('Intentional Close', {
                                statusCode: Types_1.DisconnectReason.connectionClosed
                            }));
                        };
                        cancelToken = () => onErr(new boom_1.Boom('Cancelled', {
                            statusCode: 500
                        }));
    ...
    const query = async ({
        ...
        const {
            promise,
            cancelToken
        } = waitForMessage(tag, !!requiresPhoneConnection, timeoutMs);
        try {
            await sendNode({
                json,
                tag,
        ...
        if (responseStatusCode === 599) { // the connection has gone bad
            end(new boom_1.Boom('WA server overloaded', {
                statusCode: 599,
                data: {
                    query: json,
                    response
        ...
        if (expect200 && Math.floor(responseStatusCode / 100) !== 2) {
            ...
            throw new boom_1.Boom(`Unexpected status in '${Array.isArray(json) ? json[0] : ((json === null || json === void 0 ? void 0 : json.tag) || 'query')}': ${message}(${responseStatusCode})`, {
                data: {
                    query: json,
                    response
                },
    ...
    const waitForSocketOpen = async () => {
        ...
        if (ws.readyState === ws.CLOSED || ws.readyState === ws.CLOSING) {
            throw new boom_1.Boom('Connection Already Closed', {
                statusCode: Types_1.DisconnectReason.connectionClosed
            });
        }
        let onOpen;
    ...
    ws.on(Defaults_1.PHONE_CONNECTION_CB, json => {
        if (!json[1]) {
            end(new boom_1.Boom('Connection terminated by phone', {
                statusCode: Types_1.DisconnectReason.connectionLost
            }));
    ...
    ws.on('CB:Cmd,type:disconnect', json => {
        ...
        end(new boom_1.Boom(`Connection terminated by server: "${kind || 'unknown'}"`, {
            statusCode: reason
        }));
    });
    return {
